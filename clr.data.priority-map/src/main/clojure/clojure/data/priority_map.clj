...
"}
  ...
  (:import clojure.lang.MapEntry  clojure.lang.PersistentTreeMap))     ;;; java.util.Map
...
(defn mk-bound-fn
  ...
  (fn [e] (test (.. sc comparator (Compare (. sc entryKey e) key)) 0)))                          ;;; compare 
...
(deftype PersistentPriorityMap [priority->set-of-items item->priority _meta keyfn]
  Object
  (ToString [this] (str (.seq ^clojure.lang.Seqable this)))                                                          ;;; toString + added ^PersistentPriorityMap
  ...
  (clojure.lang.IPersistentMap.assoc [this item priority]                                 ;;; clojure.lang.IPersistentMap. added
  ...
  (clojure.lang.IPersistentMap.cons [this e]                                                         ;;; clojure.lang.IPersistentMap. added
    (if (map? e)
      (into this e)
  ...
  (GetHashCode [this] (.GetHashCode item->priority))                                  ;;; hashCode .hashCode
  (Equals [this o] (or (identical? this o) (.Equals item->priority o)))               ;;; equals .Equals
  ...
  ;;; Had to add the IPersistentCollection methods because of the masking of IPersistentCollection.cons by IPersistenMap.cons   -- need to be smarter in the deftype -- someday -- major sigh
  clojure.lang.IPersistentCollection
  (count [this] (count item->priority))
  (clojure.lang.IPersistentCollection.cons [this e]
    (if (map? e)
      (into this e)
      (let [[item priority] e] (assoc this item priority))))  
  (empty [this] (PersistentPriorityMap. (empty priority->set-of-items) {} _meta keyfn))
  ...
  ;;; Similarly for Associative // assoc
  ...
  (entryAt [this k]
    (let [v (.valAt this k this)]
      (when-not (identical? v this)
        (MapEntry. k v))))
  (clojure.lang.Associative.assoc [this k v] (.assoc ^clojure.lang.IPersistentMap this k v))		
  ...
  ;;; We need to attach at attribute -- I have to look up how to do this                             ;;; java.io.Serializable  ;Serialization comes for free with the other things implemented
  clojure.lang.MapEquivalence
  
  System.Collections.IDictionary
  (get_IsFixedSize [this] false)
  (get_IsReadOnly  [this] true)
  (get_Keys [this] (keys this))
  (get_Values [this] (vals this))
  (get_Item [this key] (.valAt this key))
  (Add [this k v] (throw (NotImplementedException.)))
  (Clear [this] (throw (NotImplementedException.)))
  ...
  ;;;(put [this k v] (throw (NotImplementedException.)))                                    ;;; UnsupportedOperationException
  ;;;(remove [this k] (throw (NotImplementedException.)))                                   ;;; UnsupportedOperationException
  ;;;(putAll [this m] (throw (NotImplementedException.)))                                   ;;; UnsupportedOperationException
  ;;;(clear [this] (throw (NotImplementedException.)))                                      ;;; UnsupportedOperationException
  ;;;(keySet [this] (set (keys this)))
  ;;;(values [this] (vals this))
  ;;;(entrySet [this] (set this))
  
  System.Collections.IEnumerable                                                            ;;; Iterable
  (System.Collections.IEnumerable.GetEnumerator [this] (clojure.lang.SeqEnumerator. (seq this)))                             ;;; iterator SeqIterator.
  ...
  (peek [this]
    (when-not (= (count item->priority) 0)                           ;;; (.isEmpty this)
  ...
  (pop [this]
    (if (= (count item->priority) 0) (throw (InvalidOperationException. "Can't pop empty priority map"))                 ;;; (.isEmpty this) IllegalStateException.
        (let [f (first priority->set-of-items),
              item-set (val f)
              item (first item-set),
              priority-key (key f)]
