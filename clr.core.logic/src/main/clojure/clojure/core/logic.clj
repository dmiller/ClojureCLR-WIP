...
(defmethod print-method Pair [x ^TextWriter writer]                      ;;; ^Writer
  (.Write writer (str "(" (:lhs x) " . " (:rhs x) ")")))                 ;;; .write
...
(defn add-var [cs x c]
  (when-not (lvar? x)
    (throw (Exception. (str "constraint store assoc expected logic var key: " x))))           ;;; Error.
  (let [cm (:cm cs)
        km (:km cs)
        cid (:cid cs)
...
(deftype LVar [id unique name oname hash meta]
  ...
  Object
  (ToString [_] (str "<lvar:" name ">"))                        ;;; toString
...
(defn lvar
  ([]
     (let [id (. clojure.lang.RT (nextID))
           name (str id)]
       (LVar. id true name nil (.GetHashCode name) nil)))          ;;; hashCode
  ([name]
     (lvar name true))
  ([name unique]
     (let [oname name
           ...
           name (if unique
                  (str name "__" id)
                  (str name))]
       (LVar. id unique name oname (.GetHashCode name) nil))))     ;;; hashCode

(defmethod print-method LVar [x ^TextWriter writer]                ;;; Writer
  (.Write writer (str "<lvar:" (:name x) ">")))                    ;;; .write
...
(defmethod print-method LCons [x ^TextWriter writer]             ;;; ^Writer
  (.Write writer (str x)))                                       ;;; .write
...
(defmethod print-method AnswerCache [x ^TextWriter writer]          ;;; ^Writer
  (.Write writer (str x)))                                          ;;; .write
...
(defmacro tabled
  ...
  [args & grest]
  (let [uuid (symbol (str "tabled-" (System.Guid/NewGuid)))]                     ;;; (UUID/randomUUID)
    `(fn ~uuid [~@args]
       (let [argv# ~args]
         (fn [a#]
...
(defn run-constraint [c]
  (fn [a]  (prn "run-constraint1: " c)
    (let [c' (-step c a)]  (prn "run-constraint2: " c' (-runnable? c'))
...
(defn fix-constraints
  "A goal to run the constraints in cq until it is empty. Of
   ...
   finds the fixpoint."
  [a]   (prn "fix-constraits1: " a)
  (loop [a a]
    (when a   (prn "fix-constraits2: " a)
      (let [cq (:cq a)]
        (if (zero? (count cq))
          (assoc a :cq nil)
          (let [c (first cq)]
